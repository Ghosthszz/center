...
async function salvarCarro() {
  ...
  try {
    ...
    if (isNovo) carroEditando = { id: carrosCache.length + 1, fotos: [] };
    ...
    json.usuarios.forEach(u => {
      listaUsuarios.innerHTML += `
        <div class="item">
          ...
          <button onclick="removerUsuario('${u.uuid}')">🗑️</button>
        </div>
      `;
    });
  } finally {
...
async function removerUsuario(uuid) {
  ...
  try {
    ...
    json.usuarios = json.usuarios.filter(u => u.uuid !== uuid);
